...
export function HomeMusicPlayer() {
  ...
  // Always show after 500ms — no dependency on any envelope event
  useEffect(() => {
    const t = setTimeout(() => {
      setEntered(true);
      // Show invite toast a beat after the vinyl slides in
      setTimeout(() => showToastMessage(), 700);
    }, 500);
    return () => clearTimeout(t);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Mobile: create audio element (no pre-unlock needed — user click is the gesture)
  useEffect(() => {
    ...
    <>
      ...
      >
        ...
        {showToast && (
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', color: '#C9A684', fontSize: '0.62rem', letterSpacing: '0.18em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
              <span style={{ fontSize: '0.8rem' }}>♪</span>
              {playing ? 'Now playing' : 'Our favorite song'}
            </span>
            <span style={{ color: '#FBF7EE', fontSize: '0.75rem', fontStyle: 'italic', whiteSpace: 'nowrap' }}>
              Raindance x Meri Zindagi
            </span>
            <span style={{ color: '#8a7d6c', fontSize: '0.6rem', letterSpacing: '0.06em', whiteSpace: 'nowrap', marginTop: '0.1rem' }}>
              {playing ? 'tap vinyl to pause' : 'tap the vinyl to play'} · tap here to dismiss
            </span>
          </button>
        )}
        ...
        <button
          ...
          style={{ width: 56, height: 56, border: 'none', background: 'none', cursor: 'pointer', padding: 0, position: 'relative', flexShrink: 0 }}
        >
          <svg
            viewBox="0 0 56 56"
            width="56"
          ...
          {!playing && (
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', background: 'rgba(201,166,132,0.1)' }}>
      ...
      {loaded && !isMobile && (
        <iframe
          ...
          style={{ position: 'fixed', width: 1, height: 1, bottom: 0, right: 0, opacity: 0, pointerEvents: 'none', border: 'none', zIndex: -1 }}
        />
      )}
    </>
  );
